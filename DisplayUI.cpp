...
void DisplayUI::LedCheck(){
  ledcheck = !ledcheck;
  if(ledcheck)
    led.ledenabled = true;
  else
  {
    led.setMode(LED_MODE::OFF, true);
    led.ledenabled = false;
  }
}
void DisplayUI::setup() {
    ...
    createMenu(&mainMenu, NULL, [this]() {
        ...
        addMenuNode(&mainMenu, D_CLOCK, [this]() { // PACKET MONITOR
         ...
         addMenuNode(&mainMenu, D_STATUSLED, [this]() { // STATUS LED
            LedCheck();
        });
#ifdef HIGHLIGHT_LED
...
void DisplayUI::drawClock() {
   ...
   if(!attack.isRunning() && !hibernation) //For energy saving, hibernation
    ...
    clockTime += String(clockMinute) + ":" + String(clockSecond);
